...
switch($job){
    ...
    case 'salvar':{
        ...
        $arrGruposModulosPk = array();
        
        if($grupos_modulos_pk != "")
            $arrGruposModulosPk = json_decode ($grupos_modulos_pk, true);

        if($grupos_modulos_pk != "" && !is_array($arrGruposModulosPk)){
            $result  = 'error';
            $message = 'Falha ao interpretar os modulos informados.';
            $mysql_data = array();
            break;
        }

        $arrModulosDuplicados = array();
        $arrModulosInformados = array();
        for($i = 0; $i < count($arrGruposModulosPk); $i++){
            $modulos_pk = $arrGruposModulosPk[$i]['modulos_pk'];
            if(in_array($modulos_pk, $arrModulosInformados)){
                $arrModulosDuplicados[] = $modulos_pk;
            }
            else{
                $arrModulosInformados[] = $modulos_pk;
            }
        }

        if(count($arrModulosDuplicados) > 0){
            $result  = 'error';
            $message = 'Nao e permitido repetir modulos no mesmo grupo.';
            $mysql_data = array();
            break;
        }
