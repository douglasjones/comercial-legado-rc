...
switch($job){ 
    case 'listarQtdeLead':{
        ...
        $tempo_contrato_ini = $_REQUEST['tempo_contrato_ini'];
        $tempo_contrato_fim = $_REQUEST['tempo_contrato_fim'];
        $qtde_linhas_ini = $_REQUEST['qtde_linhas_ini'];
        $qtde_linhas_fim = $_REQUEST['qtde_linhas_fim'];
        
        $resultado = "";
        $query = $transferencia_leaddao->listarQtdeLead($polos_pk,$ds_razao_social,$ic_status_1,$ic_status_2,$ic_status_3,$ic_status_4,$grupos_pk,$usuarios_pk,$tipo_pessoa_pk,$mailing_pk,$operador_pk,$classificacao_operador_pk,$tempo_contrato_ini,$tempo_contrato_fim,$qtde_linhas_ini,$qtde_linhas_fim);
    ...
    case 'transferirLead':{
        ...
        $qtde_linhas_ini = $_REQUEST['qtde_linhas_ini'];
        $qtde_linhas_fim = $_REQUEST['qtde_linhas_fim'];
        $selected_leads_pk = $_REQUEST['selected_leads_pk'];
        
        $responsavel_pk = $_REQUEST['responsavel_pk'];
        $grupo_responsavel = $_REQUEST['grupo_responsavel'];
        
        $resultado = "";
        $query = $transferencia_leaddao->TransferirLead($polos_pk,$ds_razao_social,$ic_status_1,$ic_status_2,$ic_status_3,$ic_status_4,$grupos_pk,$usuarios_pk,$tipo_pessoa_pk,$mailing_pk,$operador_pk,$classificacao_operador_pk,$tempo_contrato_ini,$tempo_contrato_fim,$qtde_linhas_ini,$qtde_linhas_fim,$selected_leads_pk);
    ...
    case 'listarLeadsTransferencia':{
        ...
        $qtde_linhas_ini = $_REQUEST['qtde_linhas_ini'];
        $qtde_linhas_fim = $_REQUEST['qtde_linhas_fim'];
        $status_consulta = $_REQUEST['status_consulta'];
        $qtde_consulta = $_REQUEST['qtde_consulta'];

        $resultado = "";
        $query = $transferencia_leaddao->listarLeadsTransferencia($polos_pk,$ds_razao_social,$ic_status_1,$ic_status_2,$ic_status_3,$ic_status_4,$grupos_pk,$usuarios_pk,$tipo_pessoa_pk,$mailing_pk,$operador_pk,$classificacao_operador_pk,$tempo_contrato_ini,$tempo_contrato_fim,$qtde_linhas_ini,$qtde_linhas_fim,$status_consulta,$qtde_consulta);
        $arrLeadsPk = array();
        $arrResumoOperadora = array();

        if(count($query) > 0){
            for($i = 0; $i < count($query); $i++){
                $arrLeadsPk[] = $query[$i]["pk"];
            }

            $queryResumoOperadora = $transferencia_leaddao->listarResumoOperadoraPorLeads($arrLeadsPk);

            if(count($queryResumoOperadora) > 0){
                for($i = 0; $i < count($queryResumoOperadora); $i++){
                    $arrResumoOperadora[$queryResumoOperadora[$i]["leads_pk"]] = array(
                        "tempo_contrato_pk" => $queryResumoOperadora[$i]["tempo_contrato_pk"],
                        "ds_qtde_voz" => $queryResumoOperadora[$i]["ds_qtde_voz"]
                    );
                }
            }
        }
        ...
        if(count($query) > 0){
            for($i = 0; $i < count($query); $i++){
                $resumoOperadora = isset($arrResumoOperadora[$query[$i]["pk"]]) ? $arrResumoOperadora[$query[$i]["pk"]] : array(
                    "tempo_contrato_pk" => "",
                    "ds_qtde_voz" => ""
                );
                ...
                $mysql_data[] = array(
                    ...
                    "tempo_contrato_pk" => $resumoOperadora["tempo_contrato_pk"],
                    "ds_qtde_voz" => $resumoOperadora["ds_qtde_voz"],
                    "ds_classificacao" => $query[$i]["ds_classificacao"]
                );
            }
        }
