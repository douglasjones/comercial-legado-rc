...
function fcEnviar(){
    ...
    var url = '../controller/grupo.controller.php?job=salvar&token=' + encodeURIComponent(token) +
        '&pk=' + encodeURIComponent(pk) +
        '&ds_grupo=' + encodeURIComponent(v_ds_grupo) +
        '&polos_pk=' + encodeURIComponent(v_polos_pk) +
        '&grupos_modulos_pk=' + encodeURIComponent(strJSONDadosTabela);
    var request = $.ajax({
        url:          url,
        cache:        false,
        dataType:     'json',
    ...
    request.done(function(output){
        if (output.result == 'success'){
            ...
            alert(output.message || 'Falhou a requisição para salvar o registro');
        }
    });
    ...
    if(pk > 0){
        ...
        request.done(function(output){
            if (output.result == 'success'){
                ...
                $("#polos_pk").val(output.data[0]['polos_pk']);
            }
            else{
                alert('Falhar ao carregar o registro');
            }
...
function fcFormatarDadosModulos(){
    ...
    for(i = 0; i < cboModulosPk.length; i++){
        ...
        if(cboModulosPk.get(i).value == ""){
            alert("Selecione um modulo para todas as linhas antes de salvar.");
            cboModulosPk.get(i).focus();
            return false;
        }

        if(arrModulosSelecionados.indexOf(cboModulosPk.get(i).value) >= 0){
            alert("Nao e permitido repetir o mesmo modulo no grupo. Ajuste as linhas duplicadas antes de salvar.");
            cboModulosPk.get(i).focus();
            return false;
        }
        arrModulosSelecionados.push(cboModulosPk.get(i).value);
...
$(document).ready(function()
    {
        var arrCarregar = permissao("grupo", "ins");        
