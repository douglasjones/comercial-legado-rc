...
switch($job){
    ...
    case 'listarTodos':{
        ...
        $resultado = "";
        $query = $leaddao->listar_por_tipo_pessoa_pk($token,$ds_lead,$polos_pk,$ds_razao_social,$tipo_pessoa_pk,$mailing_pk,$ds_processo_pk,$processo_default_pk,$ic_cliente,$responsavel_pk,$grupos_pk,$equipes_pk);
    ...
    case 'listarCPF':{
        ...
        $resultado = "";
        if($ds_cnpj_cpf!=""){
            $query = $leaddao->listarCpfCnpj($ds_cnpj_cpf);
        }
        else{
            $mysql_data = [];
        }
