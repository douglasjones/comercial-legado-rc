...
function fcEditarContrato(objRegistro){
    ...
    if (arrCarregar.result != 'success'){            
        $('#operador_contrato_pk').prop('disabled', true);
        $('#contratos_pk').prop('disabled', true);
        $('#dt_inicio_contrato').prop('disabled', true);
        $('#dt_fim_contrato').prop('disabled', true);
        $('#ic_contrato').prop('disabled', true);
        $('#ic_aditivo').prop('disabled', true);
        $('#cmdIncluirContratosItens').prop('disabled', true);
        $('#cmdIncluirContratosEtapas').prop('disabled', true);
    }
...
function carregarListaComboProduto(){
    ...
    request.done(function(output){
        if (output.result == 'success'){
            var arrCarregar = permissao("contrato", "upd");        
            if (arrCarregar.result != 'success'){
                strComboProdutoContrato = "<select class='form-control form-control-sm' disabled onchange='carregarValorProdutoServicoContrato(this.value)' id='produtos_servicos_contrato_pk' name='produtos_servicos_contrato_pk'><option></option>";
                for(i = 0; i < output.data.length; i++){
                    strComboProdutoContrato = strComboProdutoContrato + "<option disabled value='"+output.data[i]['pk']+"'>"+output.data[i]['ds_produto_servico']+"</option>";
                }
...
function fcIncluirContratoItens(contratos_itens_pk){
    ...
    if (arrCarregar.result != 'success'){ 
        ...
        {   
            ...
            "t3":"<input type='text' disabled class='form-control form-control-sm' onchange='fcCalcularValorVlUnitContrato()' onkeypress='mascara(this,soNumeros)' id='n_qtde_contrato' size='4'/>",
            "t4":"<input type='text' disabled class='form-control form-control-sm' onchange='fcCalcularValorVlUnitContrato()' onkeypress='mascara(this,moeda)' id='vl_unit_contrato'  />",
            "t5":"<input type='text' disabled class='form-control form-control-sm' onkeypress='mascara(this,moeda)' id='vl_total_contrato' readonly/>",
        ...
        {   
            ...
            "t4":"<input type='text' class='form-control form-control-sm' onchange='fcCalcularValorVlUnitContrato()' onkeypress='mascara(this,moeda)' id='vl_unit_contrato'  />",
            "t5":"<input type='text' class='form-control form-control-sm' onkeypress='mascara(this,moeda)' id='vl_total_contrato' readonly/>",
            "t6":"<a class='function_delete' ><span><img width=16 height=16 src='../img/excluir.png'></span></a>"
        }            
        ).draw( false );
...
function fcAbrirFormNovoContrato(){
    ...
    var arrCarregar = permissao("contrato", "ins");        
    if (arrCarregar.result != 'success'){            
...
function fcRecarregarGridContratosProcessos(){
    ...
    fcRecarregarGridItensContatosProcessos();
}
...
function carregarListaComboEtapa(){
    ...
    request.done(function(output){
        if (output.result == 'success'){
            var arrCarregar = permissao("contrato", "upd");        
            if (arrCarregar.result != 'success'){
                strContratoEtapa = "<select disabled class='form-control form-control-sm' id='contratos_etapas_pk' name='contratos_etapas_pk'><option></option>";
                for(i = 0; i < output.data.length; i++){
                    strContratoEtapa = strContratoEtapa + "<option disabled value='"+output.data[i]['t_pk']+"'>"+output.data[i]['t_ds_etapa']+"</option>";
                }
...
function fcIncluirContratoEtapas(contratos_etapas_pk){
    ...
    if (arrCarregar.result != 'success'){
        ...
        {   
            ...
            "t3":"<input type='text' disabled class='form-control form-control-sm' maxlength='10' onkeypress='mascara(this,mdata)' id='dt_etapa' />",
            "t4":"<input type='text' disabled class='form-control form-control-sm' disabled id='usuario_cadastro'/>",
            "t5":"<input type='text' disabled class='form-control form-control-sm' disabled id='dt_cadastro'  />",
            "t6":"<input type='text' disabled class='form-control form-control-sm' id='ds_obs' />",
        ...
        tblContratoEtapas.on('click', '.function_delete', function () {
            ...
            var arrCarregar = permissao("contrato_esteira", "del");        
            if (arrCarregar.result != 'success'){ 
                alert("Voce não tem permissão");
                return false;
            }
